--- @meta _

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L3)
--- Template
--- @class CollectionsJournalTab : Button, CharacterFrameTabButtonTemplate

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L9)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1LeftDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L22)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1MiddleDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L31)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1RightDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L40)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1Left = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L49)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1Middle = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L58)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1Right = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L113)
--- child of CollectionsJournalTab1 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab1HighlightTexture = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L18)
--- child of CollectionsJournal
--- @class CollectionsJournal_CollectionsJournalTab1 : Button, CollectionsJournalTab
CollectionsJournalTab1 = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L9)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2LeftDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L22)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2MiddleDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L31)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2RightDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L40)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2Left = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L49)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2Middle = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L58)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2Right = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L113)
--- child of CollectionsJournalTab2 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab2HighlightTexture = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L23)
--- child of CollectionsJournal
--- @class CollectionsJournal_CollectionsJournalTab2 : Button, CollectionsJournalTab
CollectionsJournalTab2 = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L9)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3LeftDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L22)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3MiddleDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L31)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3RightDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L40)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3Left = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L49)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3Middle = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L58)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3Right = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L113)
--- child of CollectionsJournalTab3 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab3HighlightTexture = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L28)
--- child of CollectionsJournal
--- @class CollectionsJournal_CollectionsJournalTab3 : Button, CollectionsJournalTab
CollectionsJournalTab3 = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L9)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4LeftDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L22)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4MiddleDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L31)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4RightDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L40)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4Left = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L49)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4Middle = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L58)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4Right = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L113)
--- child of CollectionsJournalTab4 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab4HighlightTexture = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L33)
--- child of CollectionsJournal
--- @class CollectionsJournal_CollectionsJournalTab4 : Button, CollectionsJournalTab
CollectionsJournalTab4 = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L9)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5LeftDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L22)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5MiddleDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L31)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5RightDisabled = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L40)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5Left = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L49)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5Middle = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L58)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5Right = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L113)
--- child of CollectionsJournalTab5 (created in template CharacterFrameTabButtonTemplate)
--- @type Texture
CollectionsJournalTab5HighlightTexture = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L38)
--- child of CollectionsJournal
--- @class CollectionsJournal_CollectionsJournalTab5 : Button, CollectionsJournalTab
CollectionsJournalTab5 = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L1107)
--- child of CollectionsJournal (created in template PortraitFrameTemplate)
--- @type PortraitFrameTemplate_PortraitFrameTemplateCloseButton
CollectionsJournalCloseButton = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L1074)
--- child of CollectionsJournal (created in template PortraitFrameTexturedBaseTemplate)
--- @type Texture
CollectionsJournalBg = {}

--- [Source](https:/github.com/Gethe/wow-ui-source/blob/live/Interface/AddOns/Blizzard_Collections/Wrath/Blizzard_Collections.xml#L12)
--- @class CollectionsJournal : Frame, PortraitFrameTemplate
CollectionsJournal = {}
CollectionsJournal["CloseButton"] = CollectionsJournalCloseButton -- inherited
CollectionsJournal["Bg"] = CollectionsJournalBg -- inherited
CollectionsJournal["layoutType"] = "PortraitFrameTemplate" -- inherited

